...
Deno.serve(async (req) => {
  ...
  const { skipEmails, onlyEmails } = await req.json().catch(() => ({}));
  ...
  // Filter: either send to specific emails only, or skip already-sent ones
  let toSend;
  if (onlyEmails && onlyEmails.length > 0) {
    const onlySet = new Set(onlyEmails);
    toSend = recipients.filter(u => onlySet.has(u.email));
  } else {
    const skipSet = new Set(skipEmails || []);
    toSend = skipSet.size > 0 ? recipients.filter(u => !skipSet.has(u.email)) : recipients;
  }

  console.log(`Sending to ${toSend.length} recipients`);

  // Step 2: Send emails with delay to avoid rate limiting
  ...
  for (const u of toSend) {
    ...
    try {
      ...
      results.sent++;
      if (results.sent % 50 === 0) console.log(`Progress: ${results.sent} sent...`);
    } catch (error) {
      ...
      if (error.message?.includes('Rate limit')) {
        ...
        try {
          ...
          console.error(`❌ Failed: ${email} - ${retryError.message}`);
        }
      } else {
        results.failed++;
