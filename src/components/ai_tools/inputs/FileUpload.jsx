...
export default function FileUpload({ id, label, onFileChange }) {
    ...
    const handleFileSelect = (selectedFile) => {
        if (selectedFile) {
            // Check file size (max 20MB for reliable AI processing)
            const maxSize = 20 * 1024 * 1024; // 20MB
            if (selectedFile.size > maxSize) {
                alert('File is too large. Please select a file smaller than 20MB for reliable AI processing.');
                return;
            }
            ...
            >
                ...
                {!file ? (
                    <div className="flex flex-col items-center justify-center gap-2 text-slate-500">
                        ...
                        <p className="text-xs mt-2">Supports PDF, DOC, DOCX, TXT, PNG, JPG (max 20MB)</p>
                    </div>
                ) : (
